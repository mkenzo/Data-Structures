...
Posic insert(Lista L, Item info) {
    ...
    if (list_aux->size == 0) {
        ...
        list_aux->tail = node_aux;
    } else {
        if (isFull(L)) {
            return NIL;
        } else {
            node_aux->next = NIL;
            node_aux->prev = list_aux->tail;
            list_aux->tail->next = node_aux;
            list_aux->tail = node_aux;
        }
    }
...
Posic insertBefore(Lista L,Posic p, Item info) {
    ...
    if (list_aux->size == 1) {
        ...
        newNode->prev = NIL;
        newNode->info = info;
        list_aux->head = newNode;
        node_aux->prev = newNode;
    } else {
        newNode->next = node_aux;
...
Posic insertAfter(Lista L, Posic p, Item info) {
    ...
    if (node_aux->next == NIL) {
        newNode->next = NIL;
        newNode->prev = node_aux;
        newNode->info = info;
        node_aux->next = newNode;
        list_aux->tail = newNode;
    } else {
        newNode->next = next;
        newNode->prev = node_aux;
        newNode->info = info;
