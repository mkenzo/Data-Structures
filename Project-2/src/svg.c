...
void printtt(double x, double y, Info info, double xc, double yc, double r, void *extra) {
    char type = get_type(info);
    void* aux = get_info(info);

    if (type == 'c') {
        insertCircle(extra, aux);
    } else if (type == 'r') {
        insertRectangle(extra, aux);
    } else if (type == 'l') {
        insertLine(extra, aux);
    } else if (type == 't'){
        insertText(extra, aux);
    }
}

void write_on_SVG(CPTree b, path paths){    
    FILE* original_svg = fopen(get_path_SVG(paths), "w+");
    fprintf(original_svg, "<!-- Matheus Kenzo -->\n <svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">");

    dfs(b, printtt, original_svg);
